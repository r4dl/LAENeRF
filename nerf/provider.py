...
class NeRFDataset:
    def __init__(self, opt, device, type='train', downscale=1, n_test=10):
        ...
        if self.mode == 'colmap' and type == 'test':
            ...
            for f in tqdm.tqdm(frames, desc=f'Loading {type} data'):
                ...
                # but: ignore this for video
                if not os.path.exists(f_path) and not type in 'video':
                    continue
                ...
                if os.path.exists(f_path[:f_path.find('.')] + '_mask.png'):
                    mask = cv2.imread(f_path[:f_path.find('.')] + '_mask.png', cv2.IMREAD_UNCHANGED)
                    # TODO: Resize
    ...
    def collate(self, index):
        ...
        if len(self.depths) > 0:
            results['depth'] = self.depths[index[0]][rays['inds'].cpu()]
