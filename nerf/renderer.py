...
class NeRFRenderer(nn.Module):
    def __init__(self,
        ...
        print(f'filter close: {filter_close_point}')
    ...
    def run_cuda(self, rays_o, rays_d, dt_gamma=0, bg_color=None, perturb=False, force_all_rays=False, max_steps=1024, T_thresh=1e-4, **kwargs):
        ...
        dens_grid = kwargs['edit_grid'] if 'edit_grid' in kwargs and kwargs['edit_grid'] is not None else self.density_bitfield
        ...
        if self.training:
            ...
            xyzs, dirs, deltas, rays = raymarching.march_rays_train(rays_o, rays_d, self.bound, dens_grid, self.cascade, self.grid_size, nears, fars, counter, self.mean_count, perturb, 128, force_all_rays, dt_gamma, max_steps)
            ...
            if len(sigmas.shape) == 2:
                ...
                # perturb depth only if not distilling
                if not ('distill' in kwargs and kwargs['distill']):
                    depth = torch.clamp(depth - nears, min=0) / (fars - nears)
                # TODO: maybe this messes with different stuff
                #else:
                #    depth += nears
            ...
            while step < max_steps:
                ...
                xyzs, dirs, deltas = raymarching.march_rays(n_alive, n_step, rays_alive, rays_t, rays_o, rays_d, self.bound, dens_grid, self.cascade, self.grid_size, nears, fars, 128, perturb if step == 0 else False, dt_gamma, max_steps)
            ...
            image = image + (1 - weights_sum).unsqueeze(-1) * bg_color
            if kwargs['scale_depth']:
                depth = torch.clamp(depth - nears, min=0) / (fars - nears)
            else:
                results['t'] = weights_sum
            image = image.view(*prefix, 3)
            depth = depth.view(*prefix)
    ...
    def run_cuda_distill(self, rays_o, rays_d, edit_bitfield, dt_gamma=0, bg_color=None, perturb=False,
        ...
        if grow_grid:
            ...
            dens_bitfield = self.density_bitfield
        ...
        while step < max_steps:
            ...
            # int_edit represents the sample per ray which first intersects the edit grid
            xyzs, dirs, deltas, edit_occ = raymarching.march_rays_distill(n_alive, n_step, rays_alive, rays_t, rays_o, rays_d, self.bound,
                                                        dens_bitfield, edit_bitfield, self.cascade, self.grid_size, nears,
            ...
            raymarching.composite_rays_distill(n_alive, n_step, rays_alive, rays_t, sigmas, rgbs, deltas, weights_sum,
                                               weights_edit_sum, depth, depth_edit, image, edit_occ, T_thresh)
        ...
        if perturb_depth:
            depth += (torch.rand(depth.shape, device=device) - 0.5) * (depth.max() - depth.min()) / max_steps

        x_term = rays_o + depth[..., None] * rays_d
        image = image.view(*prefix, 3)
        depth = depth.view(*prefix)
                ...
                for zs in Z:
                    ...
                    for cas in range(self.cascade):
                        ...
                        while head < B:
                            ...
                            mask_close = ((cam_xyzs[:, :, 2] < self.min_near) & mask_z & mask_x & mask_y).sum(
                                0).reshape(-1)  # [S, N]
