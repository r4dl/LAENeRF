...
def evaluate(directory: str, scene: str, save_dir: str = None, base_images: str = None):
    ...
    # get the masks (we saved the masks as .png)
    masks = [os.path.join('scripts', 'eval', 'masks', f'{datatype}', f'{scene}', i["file_path"][9:-3]+'png') for i in file['frames']]
    ...
    # we need a list here, because not all images may exist
    errors = []

    for img_index, img in enumerate(imgs):
        # continue if the mask does not exist
        if not os.path.exists(masks[img_index]):
            continue
        ...
        errors.append((error_img.sum() / mask.sum() / 3))
