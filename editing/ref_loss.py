# taken from
# Ref-NPR
# https://github.com/dvlab-research/Ref-NPR/blob/main/exps/ref_loss.py
    ...
    def forward(
        ...
        for block in blocks[:-1]:
            ...
            if "color_patch" in loss_names:
                layers_last = all_layers[-1]

                tmpl_feats_last = torch.cat([s_feats_all[ix_map[layers_last]]], 1)
                # tmpl_feats_last = torch.cat([self.tmpl_feats_all[ix_map[ix]] for ix in layers_last], 1)
                if "online_tmp_loss" not in loss_names:
                    content_feats_last = torch.cat([content_feats_all[ix_map[layers_last]]], 1)
...
if __name__ == '__main__':
    ...
    loss = nnfm_loss_fn(outputs=fake_output, styles=fake_style, contents=fake_content,
                        loss_names=["nnfm_loss", "content_loss", "gram_loss", "color_patch"])
    ic(loss)
