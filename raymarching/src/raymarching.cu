...
template <typename scalar_t>
__global__ void kernel_march_rays_distill(
    const uint32_t n_alive,
    const uint32_t n_step,
    const int* __restrict__ rays_alive,
    const scalar_t* __restrict__ rays_t,
    ...
    const uint32_t C, const uint32_t H,
    const uint8_t * __restrict__ grid,
    const uint8_t * __restrict__ edit_grid,
    const scalar_t* __restrict__ nears,
    const scalar_t* __restrict__ fars,
    scalar_t* xyzs, scalar_t* dirs, scalar_t* deltas,
    bool* edit_occ,
    ...
    while (t < far && step < n_step) {
        ...
        const uint32_t index = level * H3 + __morton3D(nx, ny, nz);
        const bool occ = grid[index / 8] & (1 << (index % 8));
        const bool edit_grid_occ = edit_grid[index / 8] & (1 << (index % 8));
        ...
        if (occ) {
            ...
            if (edit_grid_occ) {
                // if the edit grid is occupied, save the value as true
                *edit_occ = true;
            }
            edit_occ++;
            step++;
...
void march_rays_distill(const uint32_t n_alive, const uint32_t n_step, const at::Tensor rays_alive, const at::Tensor rays_t, const at::Tensor rays_o, const at::Tensor rays_d, const float bound, const float dt_gamma, const uint32_t max_steps, const uint32_t C, const uint32_t H, const at::Tensor grid, const at::Tensor edit_grid, const at::Tensor near, const at::Tensor far, at::Tensor xyzs, at::Tensor dirs, at::Tensor deltas, at::Tensor int_edit, const at::Tensor noises) {
    static constexpr uint32_t N_THREAD = 128;

    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
    rays_o.scalar_type(), "march_rays_distill", ([&] {
        kernel_march_rays_distill<<<div_round_up(n_alive, N_THREAD), N_THREAD>>>(n_alive, n_step, rays_alive.data_ptr<int>(), rays_t.data_ptr<scalar_t>(), rays_o.data_ptr<scalar_t>(), rays_d.data_ptr<scalar_t>(), bound, dt_gamma, max_steps, C, H, grid.data_ptr<uint8_t>(), edit_grid.data_ptr<uint8_t>(), near.data_ptr<scalar_t>(), far.data_ptr<scalar_t>(), xyzs.data_ptr<scalar_t>(), dirs.data_ptr<scalar_t>(), deltas.data_ptr<scalar_t>(), int_edit.data_ptr<bool>(), noises.data_ptr<scalar_t>());
    }));
}
...
template <typename scalar_t>
__global__ void kernel_composite_rays_distill(
    const uint32_t n_alive,
    const uint32_t n_step,
    const float T_thresh,
    int* rays_alive,
    ...
    const scalar_t* __restrict__ rgbs,
    const scalar_t* __restrict__ deltas,
    scalar_t* weights_sum, scalar_t* weights_edit_sum,
    scalar_t* depth, scalar_t* depth_edit,
    const bool* __restrict__ edit_occ,
    scalar_t* image
) {
    const uint32_t n = threadIdx.x + blockIdx.x * blockDim.x;
    if (n >= n_alive) return;
    ...
    scalar_t weight_sum = weights_sum[0];
    scalar_t weight_edit_sum = weights_edit_sum[0];
    scalar_t d = depth[0];
    scalar_t d_edit = depth_edit[0];
    scalar_t r = image[0];
    scalar_t g = image[1];
    scalar_t b = image[2];
    ...
    weights_edit_sum[0] = weight_edit_sum;
    weights_sum[0] = weight_sum; // this is the thing I needed!
    depth[0] = d;
    depth_edit[0] = d_edit;
    image[0] = r;
...
void composite_rays_distill(const uint32_t n_alive, const uint32_t n_step, const float T_thresh, at::Tensor rays_alive, at::Tensor rays_t, at::Tensor sigmas, at::Tensor rgbs, at::Tensor deltas, at::Tensor weights, at::Tensor weights_edit, at::Tensor depth, at::Tensor depth_edit, const at::Tensor int_edit, at::Tensor image) {
static constexpr uint32_t N_THREAD = 128;
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
    image.scalar_type(), "composite_rays_distill", ([&] {
        kernel_composite_rays_distill<<<div_round_up(n_alive, N_THREAD), N_THREAD>>>(n_alive, n_step, T_thresh, rays_alive.data_ptr<int>(), rays_t.data_ptr<scalar_t>(), sigmas.data_ptr<scalar_t>(), rgbs.data_ptr<scalar_t>(), deltas.data_ptr<scalar_t>(), weights.data_ptr<scalar_t>(), weights_edit.data_ptr<scalar_t>(), depth.data_ptr<scalar_t>(), depth_edit.data_ptr<scalar_t>(), int_edit.data_ptr<bool>(), image.data_ptr<scalar_t>());
    }));
}