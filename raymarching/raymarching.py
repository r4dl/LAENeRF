...
class _march_rays_distill(Function):
    @staticmethod
    @custom_fwd(cast_inputs=torch.float32)
    def forward(ctx, n_alive, n_step, rays_alive, rays_t, rays_o, rays_d, bound, density_bitfield, edit_bitfield, C, H, near, far,
        ...
        dirs = torch.zeros(M, 3, dtype=rays_o.dtype, device=rays_o.device)
        deltas = torch.zeros(M, 2, dtype=rays_o.dtype, device=rays_o.device)# 2 vals, one for rgb, one for depth
        edit_occ = torch.zeros(M, dtype=torch.bool, device=rays_o.device)
        ...
        _backend.march_rays_distill(n_alive, n_step, rays_alive, rays_t, rays_o, rays_d, bound, dt_gamma, max_steps, C, H,
                            density_bitfield, edit_bitfield, near, far, xyzs, dirs, deltas, edit_occ, noises)

        return xyzs, dirs, deltas, edit_occ


march_rays_distill = _march_rays_distill.apply
...
class _composite_rays_distill(Function):
    @staticmethod
    @custom_fwd(cast_inputs=torch.float32) # need to cast sigmas & rgbs to float
    def forward(ctx, n_alive, n_step, rays_alive, rays_t, sigmas, rgbs, deltas, weights_sum, weights_edit_sum, depth, depth_edit, image, int_edit, T_thresh=1e-2):
        ''' composite rays' rgbs, according to the ray marching formula. (for inference)
        Args:
            n_alive: int, number of alive rays
            n_step: int, how many steps we march
            ...
            rgbs: float, [n_alive * n_step, 3]
            deltas: float, [n_alive * n_step, 2], all generated points' deltas (here we record two deltas, the first is for RGB, the second for depth).
            int_edit: bool, if intersects with edit grid
        In-place Outputs:
            weights_sum: float, [N,], the alpha channel
            depth: float, [N,], the depth value
            image: float, [N, 3], the RGB channel (after multiplying alpha!)
        '''
        _backend.composite_rays_distill(n_alive, n_step, T_thresh, rays_alive, rays_t, sigmas, rgbs, deltas,
                                        weights_sum, weights_edit_sum, depth, depth_edit, int_edit, image)
        return tuple()


composite_rays_distill = _composite_rays_distill.apply